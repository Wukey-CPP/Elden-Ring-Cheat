...
const char credits[] = "Calm tf down i made a auto update ive been busy with school\nso you middle aged men that dont have a job fuck off";
...
// Function to draw a red dot at the mouse cursor position
void RenderRedDot()
{
	...
	// Set the dot color
	ImU32 color = IM_COL32(255, 0, 0, 255); // Red color with full alpha

	// Set the size of the dot
	const float dotSize = 10.0f; // Diameter of the dot

	// Draw the dot
	ImGui::GetWindowDrawList()->AddCircleFilled(mousePos, dotSize / 2.0f, color);
}

// Call this function in your rendering loop
void RenderUI()
{
	ImGui::Begin("Overlay");
...
HRESULT APIENTRY MJPresent(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags) {
	...
	if (ShowMenu == true) {
		...
		ImGui::Begin("UNREALWINDOW {driver}", &isOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_MenuBar);
		...
		if (window == "window_home")
		{
			...
			ImGui::SameLine(); HelpMarker("tf you think it is spedass");
		...
		if (window == "window_themes")
		{
			...
			if (ImGui::Button("KJ", ImVec2(ImGui::GetContentRegionAvail().x-1, NULL)))
			{
				color = &black;
			}
			...
			if (ImGui::Button("Wukey", ImVec2(ImGui::GetContentRegionAvail().x-1, NULL)))
			{
				color = &white;
			}
		...
		if (showAbout)
		{
			...
			ImGui::Text("Internal elden ring cheat by Wukey");
			ImGui::Text(credits);
			ImGui::SetCursorPos(ImVec2(285, 110));
			ImGui::Text("Build ID:	8679698");
